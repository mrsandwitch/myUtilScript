...
if ssh-copy-id -i /root/.ssh/id_rsa.pub ${GIT_USER}@git.synology.com; then
	print_msg 0 "Import RSA key to git.synology.com"
else
	print_msg 1 "Import RSA key to git.synology.com"
	echo "Fail to login to git.synology.com with ID=$GIT_USER, Please double check your ID."
	echo "If you cannot import ssh key to Git server correctly, please contact dsm_buildmaster@synology.com."
	exit 1
fi
